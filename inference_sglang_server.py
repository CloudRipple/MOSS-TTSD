...
async def generate_audio_async(session, url, payload, output_path, idx):
    ...
    try:
        async with session.post(url, json=payload) as response:
            if response.status == 200:
                ...
                print(
                    f"Prompt Tokens: {response.headers.get('X-Prompt-Tokens', 'N/A')}"
                )
                print(
                    f"Completion Tokens: {response.headers.get('X-Completion-Tokens', 'N/A')}"
                )
                return True
            else:
...
def generate_multi_audio(
    url: str, host: str, port: str, jsonl: str, output_dir: str, use_normalize: bool
):
    """Wrapper function that calls the async implementation."""
    asyncio.run(
        generate_multi_audio_async(url, host, port, jsonl, output_dir, use_normalize)
    )


async def generate_multi_audio_async(
    url: str, host: str, port: str, jsonl: str, output_dir: str, use_normalize: bool
):
    if url is None:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        for idx, item in enumerate(items):
            ...
            task = generate_audio_async(session, url, payload, output_path, idx)
            tasks.append(task)
...
if __name__ == "__main__":
    ...
    generate_multi_audio(
        args.url, args.host, args.port, args.jsonl, args.output_dir, args.use_normalize
    )
